...
<div id="search-bar" class="group hidden lg:flex transition-colors items-center h-11 mr-2 rounded-lg
...
">
    ...
    <input placeholder="find…" bind:value={keywordDesktop} on:focus={() => search(keywordDesktop, true)}
               class="transition-colors pl-10 text-sm bg-transparent outline-0
             h-full w-40 active:w-60 focus:w-60 text-black/50 dark:text-white/50"
        >
    <kbd class="transition mr-2.5 pointer-events-none select-none text-[0.625rem] leading-none px-1.5 py-1 rounded-md border border-black/15 dark:border-white/20 text-black/35 dark:text-white/35 group-focus-within:opacity-0 group-focus-within:-translate-x-1">⌘K / Ctrl K</kbd>
</div>
...
top-20 left-4 md:left-[unset] right-4 shadow-2xl rounded-2xl p-2">
    ...
    <div id="search-bar-inside" class="flex relative lg:hidden transition-colors items-center h-11 rounded-xl
      bg-black/[0.04] hover:bg-black/[0.06] focus-within:bg-black/[0.06]
      dark:bg-white/5 dark:hover:bg-white/10 dark:focus-within:bg-white/10
  ">
        <Icon icon="material-symbols:search" class="absolute text-[1.25rem] pointer-events-none ml-3 transition my-auto text-black/30 dark:text-white/30"></Icon>
